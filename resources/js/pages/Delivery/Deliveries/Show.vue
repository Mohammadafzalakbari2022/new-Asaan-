...
interface DeliveryDetail {
  ...
  address: { full_name: string; full_address: string; phone: string } | null
  events: EventItem[]
}
...
const canDeliver = props.delivery.status === 'out_for_delivery' || props.delivery.status === 'arriving'
const isDone = props.delivery.status === 'delivered' || props.delivery.status === 'undelivered'

const submitDeliver = () => {
  delivering.value = true
  router.post(`/delivery/deliveries/${props.delivery.id}/deliver`, deliverForm.value, {
    onFinish: () => {
    ...
    <div class="space-y-6">
      ...
      <section class="rounded-xl border border-border bg-background p-4">
        ...
        <div class="space-y-2 text-sm">
          <p v-if="props.delivery.address?.full_name" class="flex items-center gap-2.5">
            <User class="h-4 w-4 flex-none text-muted-foreground" />
            {{ props.delivery.address.full_name }}
          </p>
          <p v-if="props.delivery.address?.full_address" class="flex items-start gap-2.5">
            <MapPin class="mt-0.5 h-4 w-4 flex-none text-muted-foreground" />
            {{ props.delivery.address.full_address }}
          </p>
          <p
            v-if="props.delivery.address?.phone || props.delivery.customer_phone"
            class="flex items-center gap-2.5"
          >
            <Phone class="h-4 w-4 flex-none text-muted-foreground" />
            {{ props.delivery.address?.phone || props.delivery.customer_phone }}
          </p>
          <div class="flex flex-wrap gap-x-6 gap-y-1 pt-1">
            <p v-if="props.delivery.scheduled_date" class="flex items-center gap-2.5">
              <Calendar class="h-4 w-4 flex-none text-muted-foreground" />
              {{ new Date(props.delivery.scheduled_date).toLocaleDateString() }}
            </p>
            <p v-if="props.delivery.cod_amount" class="flex items-center gap-2.5 font-medium text-foreground">
              <Banknote class="h-4 w-4 flex-none text-muted-foreground" /> COD ${{ props.delivery.cod_amount }}
            </p>
          </div>
        </div>
      </section>
      ...
      <!-- Proof of delivery -->
      <section v-if="isDone && props.delivery.recipient_name" class="rounded-xl border border-border bg-background p-4">
        <h2 class="mb-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
          Proof of delivery
        </h2>
        <div class="flex items-center gap-3 text-sm">
          <div class="flex h-9 w-9 flex-none items-center justify-center rounded-lg bg-primary/10">
            <Check class="h-4 w-4 text-primary" />
          </div>
          <p class="text-foreground">{{ props.delivery.recipient_name }}</p>
          <p v-if="props.delivery.cod_received" class="ml-auto font-medium text-foreground">
      ...
      <!-- Undelivered reason -->
      <section
        v-if="props.delivery.status === 'undelivered' && props.delivery.failure_reason"
        class="rounded-xl border border-border bg-background p-4"
      >
        <h2 class="mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
          Not delivered
        </h2>
        <p class="text-sm font-medium text-foreground">
          {{ props.delivery.failure_reason }}
        </p>
        <p v-if="props.delivery.failure_note" class="mt-1 text-sm text-muted-foreground">
      ...
      <section v-if="!isDone" class="space-y-2">
        <button
          ...
          @click="router.post(`/delivery/deliveries/${props.delivery.id}/start`)"
        >
        ...
        <button
          ...
          @click="router.post(`/delivery/deliveries/${props.delivery.id}/arriving`)"
        >
        ...
        <div v-if="canDeliver" class="space-y-2">
          <button
            type="button"
            class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-success px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-success/90"
      ...
      <section
        ...
        class="rounded-xl border border-border bg-background p-4"
      >
        <h2 class="mb-3 text-sm font-semibold text-foreground">Confirm delivery</h2>
        <form class="space-y-3" @submit.prevent="submitDeliver">
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground" for="recipient_name">
              Recipient
            </label>
            <input
              id="recipient_name"
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground" for="cod_received">
              Cash received (COD)
            </label>
            <div class="relative">
            ...
            <p class="mt-1 text-xs text-muted-foreground">Amount the customer handed over, if COD.</p>
          </div>

          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground" for="note">Note (optional)</label>
            <textarea
              id="note"
          ...
          <div class="flex gap-2">
            <button
              type="submit"
              :disabled="delivering"
              class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-success px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-success/90 disabled:opacity-50"
            >
      ...
      <section
        ...
        class="rounded-xl border border-border bg-background p-4"
      >
        <h2 class="mb-3 text-sm font-semibold text-foreground">Why couldn't you deliver?</h2>
        <form class="space-y-3" @submit.prevent="submitUndelivered">
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground" for="failure_reason">
              Reason
            </label>
            <select
              id="failure_reason"
            ...
            >
              ...
              <option v-for="r in props.failureReasons" :key="r.value" :value="r.value">
                {{ r.label }}
              </option>
            </select>
          </div>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground" for="failure_note">
              Details (optional)
            </label>
            <textarea
              id="failure_note"
